...
const NAV_ALL = [
  ...
  { key: "roles",       label: "Roles & Permissions", roles: ["admin"],           icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg> },
...
function AssetTransferSection({ token, companyId, switcher }) {
  ...
  const [srcCompanyId, setSrcCompanyId] = React.useState(String(companyId || ""));
  ...
  const [atResult, setAtResult]           = React.useState(null);
  ...
  // Load assets for selected source company
  React.useEffect(() => {
    setAtLoading(true);
    ...
    if (srcCompanyId) qs.append("companyId", srcCompanyId);
  ...
  }, [token, atSearch, srcCompanyId]);

  // Reload departments when source company changes
  React.useEffect(() => {
    const deptUrl = srcCompanyId
      ? `${base}/api/company-portal/departments?companyId=${srcCompanyId}`
      : `${base}/api/company-portal/departments`;
    fetch(deptUrl, { headers: H })
...
export default function CompanyEmployeePortal() {
  ...
  const [advFilterTransferred, setAdvFilterTransferred]     = useState("");
  ...
  const [bulkEditField, setBulkEditField] = useState("make");
  ...
  // Virtual scrolling for asset table
  const assetTableContainerRef = useRef(null);
  const [assetScrollTop, setAssetScrollTop] = useState(0);
  const ASSET_ROW_HEIGHT = 44; // px per row (approx)
  const ASSET_SCROLL_BUFFER = 15; // extra rows above/below viewport
  ...
  }, [nav, token, load, allCompaniesMode]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const filteredAssets = useMemo(() =>
    assets.filter((a) => {
      ...
      const matchTransferred = !advFilterTransferred ||
        (advFilterTransferred === "yes" ? ((a.transferCount || a.transfer_count || 0) > 0) : ((a.transferCount || a.transfer_count || 0) === 0));

      return matchSearch && matchStatus && matchAdvDept && matchAdvBuilding && matchAdvCategory && matchAdvMaint && matchAdvRber && matchAdvDateFrom && matchAdvDateTo && matchAdvMake && matchAdvModel && matchAdvWorkingStatus && matchAdvFloor && matchAdvRoom && matchTransferred;
    }),
    [assets, assetSearch, assetStatusFilter, advFilterDept, advFilterBuilding, advFilterCategory, advFilterMaint, advFilterRber, advFilterDateFrom, advFilterDateTo, advFilterMake, advFilterModel, advFilterWorkingStatus, advFilterFloor, advFilterRoom, advFilterTransferred]
  );

  // Reset virtual scroll position when filters change so the view starts from row 1
  useEffect(() => {
    setAssetScrollTop(0);
    if (assetTableContainerRef.current) assetTableContainerRef.current.scrollTop = 0;
  }, [assetSearch, assetStatusFilter, advFilterDept, advFilterBuilding, advFilterCategory, advFilterMaint, advFilterRber, advFilterDateFrom, advFilterDateTo, advFilterMake, advFilterModel, advFilterWorkingStatus, advFilterFloor, advFilterRoom, advFilterTransferred]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkEditAssets = async () => {
    if (!bulkEditValue.trim() || selectedQrIds.size === 0) return;
    setBulkEditSaving(true);
    const ids = [...selectedQrIds];
    let ok = 0, fail = 0;
    // Map friendly field names to metadata keys
    const META_FIELDS = { make:"make", model:"model", serialNo:"serialNo", manufacturer:"manufacturer", dealer:"dealer", accessories:"accessories", mfgYear:"manufacturingYear", invoiceNo:"invoiceNo", purchaseCost:"purchaseCost" };
    const ASSET_FIELDS = { workingStatus:"workingStatus", criticality:"criticality", departmentId:"departmentId", building:"building", floor:"floor", room:"room", status:"status" };
    const base = getApiBaseUrl();
    for (const id of ids) {
      try {
        ...
        const m = a?.metadata || {};
        let payload = {};
        if (META_FIELDS[bulkEditField]) {
          payload = { metadata: { ...m, [META_FIELDS[bulkEditField]]: bulkEditValue.trim() } };
        } else if (ASSET_FIELDS[bulkEditField]) {
          // Include existing metadata so the backend doesn't wipe other fields
          payload = { [ASSET_FIELDS[bulkEditField]]: bulkEditValue.trim(), metadata: { ...m } };
        }
        await fetch(`${base}/api/company-portal/assets/${id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    ...
    getCompanyPortalAssets(token, allCompaniesMode ? { allCompanies: "true" } : {}).then(d => d && setAssets(d)).catch(() => {});
    if (fail) alert(`${ok} updated, ${fail} failed.`);
    else alert(`${ok} asset(s) updated successfully.`);
  };
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      <main style={{ marginLeft: "240px", flex: 1, padding: "16px 32px 28px", minHeight: "100vh", minWidth: 0, overflowX: "hidden" }}>
        ...
        {nav === "dashboard" && (() => {
          ...
          return <HealthcareDashboard token={token} allCompaniesMode={allCompaniesMode} showSla={slaVisible} externalRefreshKey={hcDashRefreshKey} onOpenAsset={openAssetFromDash} onOpenParts={() => setNav("parts")} onTileNavigate={(key, filterData) => {
          ...
          <div style={{ maxWidth: "100%" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px", flexWrap: "wrap", gap: "10px" }}>
              <div>
                ...
                <p style={{ color: "#64748b", fontSize: "13.5px", margin: 0 }}>All registered assets · {filteredAssets.length} shown</p>
              </div>
              <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                {isAdmin && (<>
                  {/* Print selected */}
                  ...
                  {selectedQrIds.size > 0 && (
                    <button onClick={() => { setShowBulkEditPanel(p => !p); setBulkEditValue(""); }}
                      style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "7px 12px", borderRadius: "8px", background: showBulkEditPanel ? "#fffbeb" : "#f8fafc", color: showBulkEditPanel ? "#92400e" : "#475569", border: `1px solid ${showBulkEditPanel ? "#fde68a" : "#e2e8f0"}`, cursor: "pointer", fontSize: "12px", fontWeight: 600, whiteSpace: "nowrap" }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      Bulk Edit ({selectedQrIds.size})
                    </button>
                  )}
                  {/* Print All QR */}
            ...
            {/* Bulk Edit Panel — shown when assets selected + Bulk Edit clicked */}
            {showBulkEditPanel && selectedQrIds.size > 0 && (
              <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: "12px", padding: "16px 20px", marginBottom: "16px", display: "flex", gap: "12px", alignItems: "flex-end", flexWrap: "wrap" }}>
                <div style={{ flex: "0 0 auto" }}>
                  <div style={{ fontSize: "12px", fontWeight: 700, color: "#92400e", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Editing {selectedQrIds.size} asset(s)</div>
                </div>
                <div style={{ flex: "1 1 180px", minWidth: "160px" }}>
                  <label style={{ display: "block", fontSize: "11px", fontWeight: 700, color: "#475569", marginBottom: "4px", textTransform: "uppercase" }}>Field</label>
                  <select value={bulkEditField} onChange={e => { setBulkEditField(e.target.value); setBulkEditValue(""); }}
                    style={{ width: "100%", padding: "8px 10px", borderRadius: "8px", border: "1px solid #fde68a", fontSize: "13px", background: "#fff", outline: "none" }}>
                    <optgroup label="Equipment Details">
                      <option value="make">Make / Manufacturer</option>
                      <option value="model">Model</option>
                      <option value="serialNo">Serial No.</option>
                      <option value="dealer">Dealer / Distributor</option>
                      <option value="accessories">Accessories</option>
                      <option value="mfgYear">Manufacturing Year</option>
                      <option value="invoiceNo">Invoice No.</option>
                      <option value="purchaseCost">Purchase Cost (₹)</option>
                    </optgroup>
                    <optgroup label="Status &amp; Classification">
                      <option value="workingStatus">Working Status</option>
                      <option value="criticality">Category (Critical/Non_Critical)</option>
                      <option value="status">Asset Status (Active/Inactive)</option>
                    </optgroup>
                ...
                <div style={{ flex: "2 1 220px", minWidth: "180px" }}>
                  <label style={{ display: "block", fontSize: "11px", fontWeight: 700, color: "#475569", marginBottom: "4px", textTransform: "uppercase" }}>New Value</label>
                  {["workingStatus"].includes(bulkEditField) ? (
                  ...
                  ) : ["criticality"].includes(bulkEditField) ? (
                    <select value={bulkEditValue} onChange={e => setBulkEditValue(e.target.value)}
                      style={{ width: "100%", padding: "8px 10px", borderRadius: "8px", border: "1px solid #fde68a", fontSize: "13px", background: "#fff", outline: "none" }}>
                      <option value="">— Select —</option>
                      <option value="Critical">Critical</option>
                    ...
                    <input value={bulkEditValue} onChange={e => setBulkEditValue(e.target.value)}
                      placeholder={`Enter new value for all selected assets…`}
                      style={{ width: "100%", padding: "8px 10px", borderRadius: "8px", border: "1px solid #fde68a", fontSize: "13px", outline: "none", boxSizing: "border-box" }} />
                  )}
                </div>
                <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                  <button onClick={() => { setShowBulkEditPanel(false); setBulkEditValue(""); }}
                    style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", fontWeight: 600, fontSize: "13px", cursor: "pointer" }}>
                    Cancel
                  </button>
                  <button onClick={handleBulkEditAssets} disabled={!bulkEditValue.trim() || bulkEditSaving}
                    style={{ padding: "8px 16px", borderRadius: "8px", border: "none", background: "#f59e0b", color: "#fff", fontWeight: 700, fontSize: "13px", cursor: (!bulkEditValue.trim() || bulkEditSaving) ? "not-allowed" : "pointer", opacity: (!bulkEditValue.trim() || bulkEditSaving) ? 0.6 : 1 }}>
                    {bulkEditSaving ? "Applying…" : `Apply to ${selectedQrIds.size} Assets`}
                  </button>
                </div>
              </div>
            )}
            ...
            <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
              ...
              {showAdvancedFilter && (
                <div style={{ padding: "14px 20px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    <label style={{ fontSize: "11px", color: "#64748b", fontWeight: 600 }}>Transferred</label>
                    <select value={advFilterTransferred} onChange={e => setAdvFilterTransferred(e.target.value)}
                      style={{ padding: "5px 9px", border: "1px solid #e2e8f0", borderRadius: "7px", fontSize: "12px", background: "#fff", outline: "none" }}>
                      <option value="">All Assets</option>
                      <option value="yes">Transferred Only</option>
                      <option value="no">Never Transferred</option>
                    </select>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  ...
                  <button onClick={() => { setAdvFilterDept(""); setAdvFilterBuilding(""); setAdvFilterCategory(""); setAdvFilterMaint(""); setAdvFilterRber(""); setAdvFilterDateFrom(""); setAdvFilterDateTo(""); setAdvFilterMake(""); setAdvFilterModel(""); setAdvFilterWorkingStatus(""); setAdvFilterFloor(""); setAdvFilterRoom(""); setAdvFilterTransferred(""); }}
                    ...
                    <tbody>
                      {(() => {
                        if (filteredAssets.length === 0) {
                          return <tr><td colSpan={50} style={{ padding: "40px", textAlign: "center", color: "#94a3b8" }}>No assets found</td></tr>;
                        }
                        const containerH = assetTableContainerRef.current?.clientHeight || window.innerHeight * 0.65;
                        const vStart = Math.max(0, Math.floor(assetScrollTop / ASSET_ROW_HEIGHT) - ASSET_SCROLL_BUFFER);
                        const vEnd   = Math.min(filteredAssets.length, Math.ceil((assetScrollTop + containerH) / ASSET_ROW_HEIGHT) + ASSET_SCROLL_BUFFER);
                        const topPad    = vStart * ASSET_ROW_HEIGHT;
                        const bottomPad = Math.max(0, filteredAssets.length - vEnd) * ASSET_ROW_HEIGHT;
                        return (
                          <>
                            {topPad > 0 && <tr style={{ height: `${topPad}px` }}><td colSpan={50} /></tr>}
                            {filteredAssets.slice(vStart, vEnd).map((a, idx) => {
                          const i = vStart + idx;
                          const m = a.metadata || {};
                          ...
                          <tr key={a.id} style={{ borderBottom: "1px solid #f1f5f9", background: selectedQrIds.has(a.id) ? "#f0fdf4" : undefined }}>
                            ...
                            <td style={{ padding: "10px 14px", color: (a.isVerified || Number(a.verified) === 1) ? "#16a34a" : "#dc2626", fontFamily: "monospace", fontSize: "12px", fontWeight: 600, whiteSpace: "nowrap", cursor: "pointer", textDecoration: "underline" }}
                              ...
                              onClick={() => navigate(`/company/asset/${a.id}`)}>{a.generatedAssetId || a.assetUniqueId || "—"}</td>
                            ...
                            {bottomPad > 0 && <tr style={{ height: `${bottomPad}px` }}><td colSpan={50} /></tr>}
                          </>
                        );
                      })()}
                    </tbody>
