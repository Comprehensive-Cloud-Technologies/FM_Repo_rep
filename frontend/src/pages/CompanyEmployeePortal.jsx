...
const NAV_ALL = [
  ...
  { key: "calibration",  label: "Calibration",      roles: ["admin","supervisor"],                   icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="2"/><path d="M12 5v2M12 17v2M5 12H3M21 12h-2M7.05 7.05 5.64 5.64M18.36 18.36l-1.41-1.41M7.05 16.95l-1.41 1.41M18.36 5.64l-1.41 1.41"/></svg> },
  { key: "training",     label: "Training",          roles: ["admin","supervisor"],                   icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg> },
...
/* ─── Calibration Section ────────────────────────────────────────── */
function CalibrationSection({ token }) {
  return (
    <div style={{ position: "fixed", left: 240, top: 0, right: 0, bottom: 0, zIndex: 5, display: "flex", flexDirection: "column", background: "#f1f5f9" }}>
      <div style={{ flex: 1, overflowY: "auto", padding: "28px" }}>
...
function TrainingSection({ token }) {
  return (
    <div style={{ position: "fixed", left: 240, top: 0, right: 0, bottom: 0, zIndex: 5, display: "flex", flexDirection: "column", background: "#f1f5f9", overflowY: "auto" }}>
...
export default function CompanyEmployeePortal() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      {/* ── Calibration Section ───────────────────────────────── */}
      {nav === "calibration" && (currentUser.role === "admin" || currentUser.role === "supervisor") && (
        <CalibrationSection token={token} />
      )}

      {/* ── Training Section ──────────────────────────────────── */}
      {nav === "training" && (currentUser.role === "admin" || currentUser.role === "supervisor") && (
