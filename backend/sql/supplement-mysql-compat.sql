-- ─────────────────────────────────────────────────────────────────────────────
-- MySQL 8.0 Supplement Schema  (MySQL Community compatible – no ADD COLUMN IF NOT EXISTS)
...
CALL add_col('assets', 'open_flags_count', 'INT NOT NULL DEFAULT 0');
CALL add_col('assets', 'health_status',    "VARCHAR(20) NOT NULL DEFAULT 'green'");
CALL add_col('assets', 'risk_level',       "VARCHAR(20) NOT NULL DEFAULT 'normal'");
CALL add_col('assets', 'is_verified',      'TINYINT(1)  NOT NULL DEFAULT 0');
CALL add_col('assets', 'criticality',      'VARCHAR(30) DEFAULT NULL');
CALL add_col('assets', 'rber',             'TINYINT(1)  NOT NULL DEFAULT 0');
CALL add_col('assets', 'condemned',        'TINYINT(1)  NOT NULL DEFAULT 0');

-- companies: extra columns
CALL add_col('companies', 'enabled_modules', 'TEXT DEFAULT NULL');
CALL add_col('companies', 'logo_url',        'VARCHAR(500) DEFAULT NULL');

-- checklist_templates: extra columns
CALL add_col('checklist_templates', 'questions', 'JSON NULL');
CALL add_col('checklist_templates', 'asset_id',  'INT UNSIGNED NULL');
CALL add_col('checklist_templates', 'shift_id',  'INT UNSIGNED NULL');

-- checklist_template_questions: extra columns
CALL add_col('checklist_template_questions', 'reference_image_url', 'TEXT NULL');
CALL add_col('checklist_template_questions', 'question_image_url',  'TEXT NULL');

-- checklist_submissions: extra columns
CALL add_col('checklist_submissions', 'company_user_id',  'INT UNSIGNED NULL');
CALL add_col('checklist_submissions', 'supervisor_id',    'INT UNSIGNED NULL');
CALL add_col('checklist_submissions', 'latitude',         'DOUBLE NULL');
CALL add_col('checklist_submissions', 'longitude',        'DOUBLE NULL');
CALL add_col('checklist_submissions', 'device_ip',        'VARCHAR(64) NULL');
CALL add_col('checklist_submissions', 'location_address', 'TEXT NULL');

-- logsheet_templates: extra columns
CALL add_col('logsheet_templates', 'layout_type', "VARCHAR(20) NOT NULL DEFAULT 'standard'");
CALL add_col('logsheet_templates', 'asset_id',    'INT UNSIGNED NULL');
CALL add_col('logsheet_templates', 'frequency',   "VARCHAR(20) NOT NULL DEFAULT 'daily'");
CALL add_col('logsheet_templates', 'shift_id',    'INT UNSIGNED NULL');

-- logsheet_entries: extra columns
CALL add_col('logsheet_entries', 'company_user_id',  'INT UNSIGNED NULL');
CALL add_col('logsheet_entries', 'latitude',         'DOUBLE NULL');
CALL add_col('logsheet_entries', 'longitude',        'DOUBLE NULL');
CALL add_col('logsheet_entries', 'device_ip',        'VARCHAR(64) NULL');
CALL add_col('logsheet_entries', 'location_address', 'TEXT NULL');

-- work_orders: extra columns
CALL add_col('work_orders', 'flag_id',         'INT UNSIGNED NULL');
CALL add_col('work_orders', 'asset_name',      'VARCHAR(200) NULL');
CALL add_col('work_orders', 'location',        'VARCHAR(255) NULL');
CALL add_col('work_orders', 'issue_source',    'VARCHAR(80)  NULL');
CALL add_col('work_orders', 'company_id',      'INT UNSIGNED NULL');
CALL add_col('work_orders', 'company_user_id', 'INT UNSIGNED NULL');
